  ...
  return (
    <>
      <div style={{ paddingTop: 106, minHeight: '100vh', background: 'var(--ivory)' }}>
        ...
        <div style={{ padding: '0 60px 100px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 32 }}>
          {investments.map((inv, i) => <InvestCard key={inv.id} investment={inv} index={i} />)}
        </div>
        <style>{`
          @media (max-width: 1024px) {
            div[style*="1fr 1fr"] { grid-template-columns: 1fr !important; }
            div[style*="80px 60px"] { padding: 60px 32px !important; }
            div[style*="0 60px 100px"] { padding: 0 32px 80px !important; }
          }
        `}</style>
      </div>
      <Footer />

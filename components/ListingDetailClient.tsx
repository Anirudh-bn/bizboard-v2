...
export default function ListingDetailClient({ listing, isApproved, initialRequestStatus, isLoggedIn }: Props) {
  const [modalOpen, setModalOpen] = useState(false)
  const [budget, setBudget] = useState('')
  const [message, setMessage] = useState('')
  const [submitting, setSubmitting] = useState(false)
  const [requestStatus, setRequestStatus] = useState<string | null>(initialRequestStatus)
  ...
  const handleRequest = async () => {
    ...
    const res = await fetch('/api/requests/create', {
      ...
      body: JSON.stringify({ listingId: listing.id, budget, message }),
    })
    if (res.ok) {
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: 0, maxWidth: 1200, margin: '0 auto', padding: '60px 60px' }}>
          ...
          <div style={{ paddingRight: 60 }}>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 40 }}>
              {listing.saleType && (
                <span style={{ background: 'rgba(13,27,42,0.07)', color: 'var(--navy)', padding: '5px 12px', borderRadius: 100, fontSize: 11, fontWeight: 700, textTransform: 'uppercase' as const }}>
                  {listing.saleType}
                </span>
              )}
